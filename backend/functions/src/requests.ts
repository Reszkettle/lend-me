...
export const onWriteUpdateDisplayedTitleAndSubtitleOnTile = functions.firestore
...
async function acceptRequest(
	...
	if (request == null) {
		throw new functions.https.HttpsError(
			"not-found",
			"Request with given id doesn't exist"
		);
	}

	const item = (
		await firestore.collection("items").doc(request.itemId).get()
	).data() as Item;

	if (item == null) {
		throw new functions.https.HttpsError(
			"not-found",
			"Item associated with request not found"
		);
	}
...
async function acceptExtend(request: Request, _uid: string) {
	...
	if (pendingRentalsId.length > 1) {
		throw new functions.https.HttpsError(
			"internal",
			"Multiple pending rentals found for this item"
		);
	}
	if (pendingRentalsId.length == 0) {
		throw new functions.https.HttpsError(
			"internal",
			"Unable to extend time, because no pending rental found"
		);
	}
...
async function rejectRequest(
	...
	if (item.ownerId != uid) {
		throw new functions.https.HttpsError(
			"permission-denied",
			"Not permitted to reject request"
		);
	}
...
async function updateStatusAndMessage(
	requestId: string,
