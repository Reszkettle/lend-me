...
async function getFullname(userId: string): Promise<string> {
  const user = (await firestore.collection("users").doc(userId).get()).data() as User;
...
async function updateFullnames(rentalId: string, borrowerName: string, ownerName: string) {
...
async function getLentById(itemId: string): Promise<string|null> {
  const pendingRentals = (await firestore.collection("rentals")
      ...
      .get()).docs.map((it)=>it.data()) as Array<Rental>;
  ...
  if (pendingRentals.length > 1) {
    throw new functions.https.HttpsError("internal", "Multiple pending rentals for single item");
  }
